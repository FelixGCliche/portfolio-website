...
export const TokenPreview = () => (
  <div class="space-y-10 py-6">
    <div>
      <h1 class="font-grotesk-mono mb-1 text-3xl font-bold text-balance">Design Tokens</h1>
      ...
      <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
        <For each={colorTokens}>
          {(token) => (
            <div
              class={[
                'border-border flex h-24 flex-col justify-between rounded-lg border p-3',
                token.bg,
                token.fg,
              ]}
            >
              <span class="font-grotesk-mono text-xs break-words">{token.name}</span>
            </div>
          )}
        </For>
      </div>
    ...
    <section>
      ...
      <div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
        <For each={outlineTokens}>
          {(token) => (
            <div class={['flex h-16 items-center justify-center rounded-lg border-2', token.class]}>
      ...
      <div class="flex flex-wrap gap-4">
        <For each={radiusTokens}>
          {(token) => (
            <div
              class={[
                'border-border bg-card flex h-20 w-20 items-center justify-center border-2 text-center',
                token.class,
              ]}
            >
              <span class="font-grotesk-mono text-xs">{token.name}</span>
